...
async function startDailyChallengeBySubject(subjectId, requestedCount) {
  ...
  function ensureShell() {
    ...
    if (!body.dataset.page) body.dataset.page = PAGE;
    let root = document.getElementById('pageRoot');
    if (!root) {
      const main = document.querySelector('main') || document.body;
      main.innerHTML = '<div class="pn-mounted-shell"><div id="pageRoot"></div></div>';
      root = document.getElementById('pageRoot');
    }
    if (!document.getElementById('adminBackdrop')) {
      const adminBackdrop = document.createElement('div');
      adminBackdrop.id = 'adminBackdrop';
      adminBackdrop.className = 'admin-backdrop';
      document.body.appendChild(adminBackdrop);
    }
    // patch nav links on existing design without replacing shell
    const links = [...document.querySelectorAll('a[href="#"], a[href=""]')];
    links.forEach((a) => {
      const txt = (a.textContent || '').trim().toLowerCase();
      if (txt.includes('home')) a.setAttribute('href','./index.html');
      else if (txt.includes('subject') || txt.includes('curriculum') || txt.includes('question bank') || txt.includes('topic sets')) a.setAttribute('href','./subjects.html');
      else if (txt.includes('dashboard') || txt.includes('exam dashboard') || txt.includes('performance')) a.setAttribute('href','./dashboard.html');
      else if (txt.includes('saved') || txt.includes('bookmark')) a.setAttribute('href','./saved.html');
      else if (txt.includes('final exam') || txt.includes('exams') || txt.includes('mock exams')) a.setAttribute('href','./final-exam.html');
      else if (txt.includes('review')) a.setAttribute('href','./review.html');
    });
  }
...
function startExamEngine(container, questions, minutes) {
  ...
  async function boot() {
    ...
    else if (PAGE === 'auth') document.getElementById('pageRoot').innerHTML = '<div class="card"><div class="section-header"><div><h2>Auth</h2><p>This page shell is ready. Authentication can be connected later without changing the mounted study logic.</p></div></div><div class="action-row" style="justify-content:flex-start;"><a class="btn btn-dark" href="./index.html">Back Home</a></div></div>';
